...
- `sekimore-gw` agent-setup script (`/usr/local/bin/sekimore-agent-setup.sh`) と
  ...
  どちらも **同じ `sekimore-gw` イメージ** (`ARG SEKIMORE_GW_IMAGE`、既定 `ghcr.io/amakata/sekimore-gw:0.2.5`)
...
| --- | --- | --- |
| 0.2.6 以降 | `0.2.5` (`ARG SEKIMORE_GW_IMAGE`) | CLI が英語 / 日本語を切り替える (`SEKIMORE_LANG`、既定は英語)。`sekimore guide --lang en\|ja`。AI 向けの指示は英語で置く。`--build-arg SEKIMORE_GW_IMAGE=...` で差し替え可 |
