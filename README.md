...
- The `sekimore-gw` agent-setup script (`/usr/local/bin/sekimore-agent-setup.sh`),
  ...
  in the Dockerfile — currently `ghcr.io/amakata/sekimore-gw:0.2.15` — and by nothing
...
`ghcr.io/amakata/sekimore-gw:0.2.15`. That ARG is the only thing that decides it, and a
...
`mise run gw:recreate`; there is no need to wait for this image. The two are apart at the
moment — this image takes 0.2.15 and the gateway is at 0.2.28 — and the next base release
takes 0.2.29. What each gateway version asks of you is in [UPGRADING.md](UPGRADING.md);
most ask nothing.
