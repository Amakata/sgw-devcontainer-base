...
- `sekimore-gw` agent-setup script (`/usr/local/bin/sekimore-agent-setup.sh`) と
  ...
  どちらも **同じ `sekimore-gw` イメージ** (`ARG SEKIMORE_GW_IMAGE`、既定 `ghcr.io/amakata/sekimore-gw:0.1.3`)
...
| --- | --- | --- |
| main 以降 | `0.1.3` (`ARG SEKIMORE_GW_IMAGE`) | relay 同梱。`--build-arg SEKIMORE_GW_IMAGE=...` で差し替え可 |
