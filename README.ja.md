...
**AI に開発環境を渡しても、GitHub の全権限と鍵は渡さないための devcontainer。**

AI エージェントに作業させたい。でもトークンを渡せば `repo` スコープ —
**見えるリポジトリすべての読み書き**が渡りますし、ターミナルを持つ AI は
`~/.ssh` も `.env` も読めます。

VS Code の Dev Containers で環境をコンテナに閉じ込め、**外向きの通信を関所
(sekimore-gw) に通す**ことで、資格情報をエージェントの外に置いたまま作業させます。
エージェントが持つのは、関所にしか通用しない使い捨ての鍵だけです。

GitHub の操作は `sekimore` コマンドで行い、**どの操作を許すかは案件ごとに選べます**
(`pr:create` は許可、`pr:merge` は拒否、など 26 種類)。`gh` は入れていません —
トークンを持たせると関所を素通りして、その設定がどれも効かなくなるからです。

このイメージは、その構成の dev 側の土台です。

## これで何ができるか

| | |
|---|---|
| **鍵を渡さずに git が使える** | AI が持つのは関所にしか通じない使い捨ての鍵。GitHub に届くのは、関所があなたの鍵で繋ぎ直したものです |
| **GitHub の操作を1つずつ許せる** | `pr:merge` は禁止、`issue:create` は許可、といった粒度。案件のリポジトリの外へは届きません |
| **許可した先にしか出られない** | 許可していないドメインは名前が引けず、IP を直に指定してもファイアウォールが落とします |
| **持ち出しに上限がある** | 関所が扱う宛先への送信量を数え、超えたら切って記録します |
| **コミットが Verified になる** | 署名鍵は関所側にあり、dev からは署名を頼めるだけ。鍵そのものは取り出せません |
| **毎回のビルドが速い** | 言語ランタイム以外の共通部分を焼き込んであります |

関所そのものの説明は [sekimore-gw](https://github.com/Amakata/sekimore-gw) にあります。
...
FROM ghcr.io/amakata/sgw-devcontainer-base:0.2.32
...
(現在 `ghcr.io/amakata/sekimore-gw:0.2.34`) で、ここが唯一の出どころ。
...
`ghcr.io/amakata/sekimore-gw:0.2.34`。ローカルビルドでは
...
いまは揃っている (このイメージも gateway も 0.2.34)。gateway の各版が何を要求するかは
